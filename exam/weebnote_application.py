...
def open_app():
    ...
    if choice == "Welcome Page":
        ...
        if "chosen_anime" in st.session_state:
            del st.session_state["chosen_anime"]
        #create header image
        st.image("images/night.png", use_column_width=True, width=10)
        # create a header with the capitalized current username
        st.header(f"Welcome, {st.session_state.current_user.capitalize()}!",divider="red")


        st.markdown('<p style="font-weight: bold; font-size: 24px;">Overview of functions</p>', unsafe_allow_html=True)
        st.markdown('<p style="font-size: 18px;">Generate Anime: recieve personalized recomendations based on your chosen filters</p>', unsafe_allow_html=True)
        st.markdown('<p style="font-size: 18px;"> My anime: view your saved anime titles </p>', unsafe_allow_html=True)
        st.markdown('<p style="font-size: 18px;"> My friends: check out if you and your friends match with any saved titles </p>', unsafe_allow_html=True)
