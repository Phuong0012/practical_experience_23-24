...
def login_form():
    st.image("images/night.png", use_column_width=True)
    ...
    with st.form("Login"):
        ...
        st.markdown('<p style="font-weight: bold; font-size: 15px;">3 reasons why you should get a Weebnote account now:</p>', unsafe_allow_html=True)
        st.markdown('<p style="font-size: 15px;">1. When you are feeling empty after finishing another great show, you have a place to turn to</p>', unsafe_allow_html=True)
        st.markdown('<p style="font-size: 15px;">2. You can bond with friends through our "matching" system.</p>', unsafe_allow_html=True)
        st.markdown('<p style="font-size: 15px;">3. We are not only an ANIME GENERATOR, we are a community. </p>', unsafe_allow_html=True)
